...
<div class="page-wrap">
    ...
    <div class="panel-card">
        ...
        <div class="panel-header">
            <div>
                <h3 class="panel-title">Gestión de Administradores</h3>
                <p class="panel-subtitle">Admins por local del sistema</p>
            </div>
...
<!-- MODAL -->
<div class="modal-pro" id="modalAdmin">
    <div class="modal-card">

        <div class="modal-head">
            <h4 id="tituloModal">Nuevo Admin</h4>
    ...
    <select id="id_local" class="select-pro"></select>
...
async function cargarLocales(){
    ...
    localesGlobal = json.data || [];

    const select = document.getElementById("id_local");
...
async function cargarAdmins(){
    ...
    document.getElementById("kpiActivos").innerText = data.filter(a=>a.activo==1).length;
    document.getElementById("kpiInactivos").innerText = data.filter(a=>a.activo==0).length;
    ...
    data.forEach((a,i)=>{
        ...
        tbody.innerHTML += `
            <tr>
                <td>${i+1}</td>
                <td class="local-name">${a.usuario}</td>
                <td>${a.correo || '-'}</td>
                <td>${a.nombre_local || '-'}</td>
                <td>
                    <span class="badge-state ${a.activo==1 ? 'badge-active':'badge-inactive'}">
                ...
                <td>
                    <div class="actions">
                        <button class="btn-action" onclick='editar(${JSON.stringify(a)})'>Editar</button>
                        <button class="btn-action btn-danger" onclick="eliminar(${a.id_usuario})">Desactivar</button>
                    </div>
                </td>
            </tr>
        `;
...
function editar(a){
    ...
    document.getElementById("id_admin").value = a.id_usuario;
    document.getElementById("usuario").value = a.usuario;
    document.getElementById("correo").value = a.correo || "";
    document.getElementById("password").value = "";
    document.getElementById("id_local").value = a.id_local;
...
async function guardar(){
    ...
    if(id){
        url = `${API}/admins/${id}`;
        method = "PUT";
        delete data.password; // no cambiar password aquí
    }
